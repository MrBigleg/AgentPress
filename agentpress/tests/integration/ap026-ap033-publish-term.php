...
/**
 * Assert condition for AP-026/033.
 *
...
// 1. Snapshot initial user session for clean restoration.
$initial_user_id = get_current_user_id();

// 2. Clean up any stale AP-026 fixtures from an interrupted prior run.
$old_user = get_user_by( 'login', 'agentpress_ap026_admin' );
if ( is_object( $old_user ) ) {
	$old_uid = (int) $old_user->ID;
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}agentpress_changes WHERE actor_user_id = %d", $old_uid ) );
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}agentpress_change_sets WHERE initiator_user_id = %d", $old_uid ) );
	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}agentpress_audit_events WHERE user_id = %d", $old_uid ) );
...
// 3. Create synthetic test user and draft page.
...
$publish_input = array(
	'content_id'      => $services_id,
	'idempotency_key' => 'ap026-publish',
);
...
agentpress_ap026_error(
	$publish_ability->execute(
		array(
			'content_id'      => $services_id,
			'idempotency_key' => 'ap026-republish',
		)
	),
	'AP_STATE_CONFLICT',
	'Republish was accepted.'
);
...
$fresh_stage = $create_term_ability->execute(
	array(
		'taxonomy'        => 'post_tag',
		'name'            => 'AP026 Logout',
		'idempotency_key' => 'ap026-logout',
	)
);
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}agentpress_changes WHERE actor_user_id = %d", $admin ) );
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}agentpress_change_sets WHERE initiator_user_id = %d", $admin ) );
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}agentpress_audit_events WHERE user_id = %d", $admin ) );
